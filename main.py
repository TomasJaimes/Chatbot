...
#FUNCIONES
def configurar_pagina(): #ESTA FUNCION USA STREAMLIT PARA REALIZAR EL INTERFAZ DE LA PAGINA, TAMBIEN RETORNA EL MODELO ELEGIDO POR EL USUARIO
    st.set_page_config(page_title="Chimp.AI", page_icon= "🦧")
    st.title("¡Bienvenido a Chimp.AI🙈🙉!")
...
def crear_usuario(): #ESTA FUNCION LLAMA A st.secrets PARA OBTENER LA CLAVE DE LA API DE GROQ Y CREA UN USUARIO.
    clave_secreta = st.secrets["CLAVE_API"]
    return groq.Groq(api_key = clave_secreta)

usuario = crear_usuario()

def configurar_modelo(cliente, modelo_elegido, prompt_usuario): #ESTA FUNCION CONFIGURA EL MODELO DE LENGUAJE PARA QUE PROCESE EL PROMPT DEL USUARIO
...
def inicializar_estado(): 
    if "mensajes" not in st.session_state:
        st.session_state.mensajes = [] # --> GUARDA LOS DATOS DEL USUARIO
